...
\textbf{Data cleaning.}
Two issues were identified and addressed. First, four samples had zero-valued dimensions ($x = 0$, $y = 0$, or $z = 0$), which are physically impossible for a real diamond (Figure~\ref{fig:scatter}). Second, one sample contained an extreme outlier ($y = 58.9\,\text{mm}$), far beyond the plausible range. These five rows (0.05\% of the data) were removed as they represent physically invalid or implausible measurements, leaving 9{,}995 training samples.
...
% ══════════════════════════════════════════════════════════════════════════════
	...
	extbf{Preprocessing.}
Categorical features (cut, color, clarity) were one-hot encoded, which is required for sklearn models as tree-based estimators cannot consume raw string categories. Numerical features were median-imputed as a defensive step to ensure robustness to potential missing values in unseen data. Feature standardisation (zero mean, unit variance) was applied primarily to support linear baselines such as Ridge regression; for tree-based models, scaling is not strictly necessary but does not affect correctness. All preprocessing steps were embedded inside an sklearn 	exttt{Pipeline} to prevent data leakage between cross-validation folds.
...
To further assess generalisation, the final tuned GBR pipeline was evaluated using out-of-fold (OOF) predictions from cross-validation, confirming that performance remains consistent when each sample is predicted by a model that did not see it during training.
