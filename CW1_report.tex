...
For the selected GBR model, key hyperparameters were: \texttt{n\_estimators} $= 500$ (sufficient trees for convergence), \texttt{learning\_rate} $= 0.03$ (slow learning to avoid overfitting), and \texttt{max\_depth} $= 3$ (shallow trees to promote generalisation). The low learning rate paired with a high number of estimators follows the established principle that many weak learners combined carefully outperform fewer strong learners.
